...
namespace PWMHelper
{
    ...
    class Program
    {
        ...
        private static int ParseArguments(string[] args)
        {
            ...
            {
                //Pacemaker.Program.Toast("PWM", string.Format(MessageString.InvalidValue, MinFrequency, MaxFrequency));
        ...
        private static byte[] ReadDataFromDriver()
        {
            ...
            if (error != 0)
            {
                //Pacemaker.Program.Toast("PWM", string.Format(MessageString.ErrorReadingData, error));
        ...
        private static void SetNewFrequency(byte[] baseData, int targetPwmFrequency)
        {
            ...
            if (error != 0)
                //Pacemaker.Program.Toast("PWM", string.Format(MessageString.ErrorWritingData, targetPwmFrequency, error));
